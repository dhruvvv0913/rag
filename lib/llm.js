...
// One chat turn. `tools` and `responseSchema` (§6) get implemented in later phases.
export async function complete({ system, messages }) {
  const provider = process.env.LLM_PROVIDER || 'gemini'
  if (provider === 'gemini') return geminiComplete({ system, messages })
...
async function geminiComplete({ system, messages }) {
